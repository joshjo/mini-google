...
private:
    ...
    {
        map<string, WordDoc *> tempMap;
        int start, initLine, spaces;
        ifstream inputFile;
        ...
        if (inputFile)
        {
            ...
            for (string line; getline(inputFile, line); )
            {
                ...
                else if (line.find(endLine) != string::npos)
                {
                    ...
                    multimap.insert(tempMap.begin(), tempMap.end());
                    // for (auto it = tempMap.begin(); it != tempMap.end(); it ++) {
                    //     if (it->second->pagerank > 1) {
                    //         // cout << it->second->pagerank << " - " << it->first << endl;
                    //     }
                ...
                else
                {
                    ...
                    while ((pos = line.find(" ")) != string::npos) {
                        ...
                        //count += pos;
                        if (temp.length() > 1) {
                            auto it = tempMap.find(temp);
                            if (it != tempMap.end()) {
                                it->second->inc();
                            } else {
                                tempMap[temp] = new WordDoc(idFile);
                            }
                            // tmp[&tmp];
